...
const std::unordered_map<std::string, StringList> FIXED_MENUS = {
	...
	{"compendium_menu",
	 {"COMPENDIUM_ATLAS", "COMPENDIUM_BESTIARY", "COMPENDIUM_GAZETTEER", "COMPENDIUM_MUSEUM", "COMPENDIUM_SPELLBOOK",
	  "COMPENDIUM_RETURN"}},

	{"camp_menu", {"CAMP_INSPECT", "CAMP_REORDER", "CAMP_OPTIONS", "CAMP_QUIT", "CAMP_LEAVE"}},
	...
	{"chest_menu",
	 {"CHEST_MENU_OPEN", "CHEST_MENU_INPSECT", "CHEST_MENU_CALFO", "CHEST_MENU_DISARM", "CHEST_MENU_LEAVE"}},

	{"chest_trap_menu",
	 {"CHEST_POISON_NEEDLE", "CHEST_GAS_BOMB", "CHEST_CROSSBOW_BOLT", "CHEST_EXPLODING_BOX", "CHEST_SPLINTERS",
	  "CHEST_BLADES", "CHEST_STUNNER", "CHEST_TELEPORTER", "CHEST_MAGE_BLASTER", "CHEST_PRIEST_BLASTER", "CHEST_ALARM",
	  "CHEST_TRAP_LEAVE"}},
	...
	{"castle_menu", {"CASTLE_TAVERN", "CASTLE_INN", "CASTLE_SHOP", "CASTLE_TEMPLE", "CASTLE_EDGE_OF_TOWN"}},

	{"rest_menu", {"STAY_1", "STAY_2", "STAY_3", "STAY_4", "STAY_5", "STAY_RETURN"}},

	{"store_menu", {"STORE_BUY", "STORE_SELL", "STORE_UNCURSE", "STORE_IDENTIFY", "STORE_POOL", "STORE_RETURN"}},

	{"tavern_menu",
	 {"TAVERN_ADD_TO_PARTY", "TAVERN_REMOVE_FROM_PARTY", "TAVERN_REORDER_PARTY", "TAVERN_DIVVY_GOLD", "TAVERN_CASTLE"}},

	{"edge_menu",
	 {"EDGE_OF_TOWN_TRAIN", "EDGE_OF_TOWN_MAZE", "EDGE_OF_TOWN_RESTART", "EDGE_OF_TOWN_CASTLE",
	  "EDGE_OF_TOWN_LEAVE_GAME"}},

	{"atlas_menu",
	 {"ATLAS_MENU_B1F", "ATLAS_MENU_B2F", "ATLAS_MENU_B3F", "ATLAS_MENU_B4F", "ATLAS_MENU_B5F", "ATLAS_MENU_B6F",
	  "ATLAS_MENU_B7F", "ATLAS_MENU_B8F", "ATLAS_MENU_B9F", "ATLAS_MENU_B10F", "ATLAS_RETURN"}},

	{"training_menu",
	 {"TRAINING_GROUNDS_CREATE", "TRAINING_GROUNDS_EDIT", "TRAINING_GROUNDS_DELETE", "TRAINING_GROUNDS_ROSTER",
	  "TRAINING_GROUNDS_RETURN"}},

	{"edit_menu", {"EDIT_CHANGE_NAME", "EDIT_CHANGE_CLASS", "EDIT_LEGATE_CHARACTER", "EDIT_RETURN"}},

	{"main_menu",
	 {"MAIN_MENU_OPTION_START", "MAIN_MENU_OPTION_CONTINUE", "MAIN_MENU_OPTION_OPTIONS", "MAIN_MENU_OPTION_COMPENDIUM",
	  "MAIN_MENU_OPTION_LICENSE", "MAIN_MENU_OPTION_EXIT"}},

	{"race_menu",
	 {"CHARACTER_RACE_HUMAN", "CHARACTER_RACE_ELF", "CHARACTER_RACE_DWARF", "CHARACTER_RACE_GNOME",
	  "CHARACTER_RACE_HOBBIT", "CHARACTER_RACE_RETURN"}},

	{"alignment_menu",
	 {"CHARACTER_ALIGNMENT_GOOD", "CHARACTER_ALIGNMENT_NEUTRAL", "CHARACTER_ALIGNMENT_EVIL",
	  "CHARACTER_ALIGNMENT_RETURN"}},

	{"class_menu",
	 {"CHARACTER_CLASS_FIGHTER", "CHARACTER_CLASS_MAGE", "CHARACTER_CLASS_PRIEST", "CHARACTER_CLASS_THIEF",
	  "CHARACTER_CLASS_BISHOP", "CHARACTER_CLASS_SAMURAI", "CHARACTER_CLASS_LORD", "CHARACTER_CLASS_NINJA",
	  "CHARACTER_CLASS_RETURN"}},

	{"modal_elevator_top", {"ELEVATOR_A", "ELEVATOR_B", "ELEVATOR_C", "ELEVATOR_D", "ELEVATOR_LEAVE"}},

	{"modal_elevator_bottom",
	 {"ELEVATOR_A", "ELEVATOR_B", "ELEVATOR_C", "ELEVATOR_D", "ELEVATOR_E", "ELEVATOR_F", "ELEVATOR_LEAVE"}},

	{"top_elevator_menu", {"ELEVATOR_A", "ELEVATOR_B", "ELEVATOR_C", "ELEVATOR_D", "ELEVATOR_LEAVE"}},
...
auto Sorcery::MenuBuilder::_load_roster_characters(std::vector<std::string> &items, std::vector<int> &data) -> void {
	...
	for (auto it = _ctx.game->characters.cbegin(); it != _ctx.game->characters.cend(); ++it)
		sorted.push_back(it);
...
auto Sorcery::MenuBuilder::_load_retrainable_characters(std::vector<std::string> &items, std::vector<int> &data)
	...
	sorted.reserve(_ctx.game->characters.size());
	for (auto it = _ctx.game->characters.begin(); it != _ctx.game->characters.end(); ++it)
		sorted.push_back(it);
	std::ranges::sort(sorted, {}, [](const auto &it) {
	...
	for (const auto &it : sorted) {
		...
		const auto possible_classes{character.create().get_possible_classes_display()};
...
auto Sorcery::MenuBuilder::_load_party_characters(std::vector<std::string> &items, std::vector<int> &data,
	...
	for (const auto id : party) {
		...
		const auto name_str{flags & (MENU_FULL_NAME | MENU_SHOW_POSITION) ? character.full_desc_text()
																		  : character.get_name()};
...
auto Sorcery::MenuBuilder::_load_maze_characters(std::vector<std::string> &items, std::vector<int> &data) -> void {
	...
	for (auto &[id, character] : _ctx.game->characters) {
		if (character.get_location() == Enums::Character::Location::MAZE &&
			...
			items.emplace_back(std::format("{:^24}", character.full_desc_text()));
			data.emplace_back(id);
		}
	}
}
...
auto Sorcery::MenuBuilder::_load_sick_characters(std::vector<std::string> &items, std::vector<int> &data) -> void {
	...
	for (auto &[id, character] : _ctx.game->characters) {
		if (character.get_status() == ASHES || character.get_status() == DEAD || character.get_status() == HELD ||
			character.get_status() == STONED) {

			items.emplace_back(std::format("{:<16} {:>8}", character.get_name(), character.get_status_string()));
			data.emplace_back(id);
		}
	}
}
...
auto Sorcery::MenuBuilder::_load_buy_menu(unsigned int width, std::vector<std::string> &items, std::vector<int> &data)
	...
	const auto char_id{_ctx.controller->get_character(Enums::CharacterSlot::STORE)};
	...
	for (const auto &item_type : _ctx.resources->items->get_all_types()) {
		...
		const auto in_stock{_ctx.game->state->check_shop_stock(item_type.get_type_id()) != 0};
		const auto will_sell{_ctx.game->state->check_shop_will_sell(item_type.get_type_id())};
		if (in_stock && will_sell) {

			const auto suffix{item_type.is_class_usable(character.get_class()) ? enum_name(item_type.get_category())
																			   : " (Not Usable)"};
			const auto price{item_type.get_value()};
			const auto entry{std::format("{:<20} {:>6} {:<13}", item_type.get_known_name(), price, suffix)};
...
auto Sorcery::MenuBuilder::build(const std::string &menu_name, unsigned int width, std::vector<std::string> &items,
	...
	if (menu_name == "choose_menu" || menu_name == "inspect_menu" || menu_name == "remove_character_menu" ||
		...
		_load_possible_classes(menu_name, items, data, Enums::CharacterSlot::EDIT);
		_load_fixed_menu(menu_name, width, items);
		...
		_load_sick_characters(items, data); // MENU_FULL_NAME
		_load_fixed_menu(menu_name, width, items);
	} else if (menu_name == "identify_menu" || menu_name == "drop_menu" || menu_name == "trade_menu" ||
			   menu_name == "use_menu" || menu_name == "invoke_menu" || menu_name == "equip_menu" ||
			   menu_name == "remove_item_menu") {

		_load_character_items(menu_name, items, data, Enums::CharacterSlot::INSPECT);
		_load_fixed_menu(menu_name, width, items);
	} else if (menu_name == "sell_menu") {
		_load_character_items(menu_name, items, data, Enums::CharacterSlot::STORE);
		_load_fixed_menu(menu_name, width, items);
		...
		_load_fixed_menu(menu_name, width, items);
	} else if (menu_name == "top_elevator_menu" || menu_name == "bottom_elevator_menu") {
		_load_fixed_menu(menu_name, width, items);
	} else
		_load_fixed_menu(menu_name, width, items);
}
...
auto Sorcery::MenuBuilder::_load_character_items(std::string_view menu_name, std::vector<std::string> &items,
	...
	const auto chance{character.abilities().at(Enums::Character::Ability::IDENTIFY_ITEMS)};
	...
	for (const auto &item : character.inventory.items()) {
		...
		if (flags & MENU_IDENTIFY_ITEM) {
			if (item.get_known()) {
				line = std::format("{}){}{:<16}", slot, flag, item.get_display_name());
			} else {
				line = std::format("{}){}{:<16} {:>5}%", slot, flag, item.get_display_name(), chance);
			}
		}
		// EQUIP
		...
		// USE
			...
			if (item.get_known() && item.get_usable()) {
				auto usage{_ctx.resources->items->get_item_type(item.get_type_id()).get_usage()};
				if (!usage.empty())
					usage = usage.substr(6);

				line = std::format("{}){}{:<16} {:>16}", slot, flag, item.get_display_name(), usage);
			} else {
				line = std::format("{}){}{:<16}", slot, flag, item.get_display_name());
			}
		}
		// SELL
			...
			const auto sell_to_shop_value{
				item.get_known() ? _ctx.resources->items->get_item_type(item.get_type_id()).get_value() / 2 : 1};
			line = std::format("{}){}{:<16} {:>8} G.P.", slot, flag, item.get_display_name(), sell_to_shop_value);
		} // Identify
		else if (flags & MENU_SHOP_IDENTIFY_ITEM) {
			const auto sell_to_shop_value{_ctx.resources->items->get_item_type(item.get_type_id()).get_value()};
			line = std::format("{}){}{:<16} {:>8} G.P.", slot, flag, item.get_display_name(), sell_to_shop_value);
		} // UNCURSE
		...
		// INVOKE
			...
			if (item.get_known() && item.get_usable()) {
				auto invokage{_ctx.resources->items->get_item_type(item.get_type_id()).get_invokage()};
				if (!invokage.empty())
					invokage = invokage.substr(6);

				line = std::format("{}){}{:<16} {:>16}", slot, flag, item.get_display_name(), invokage);
			} else {
				line = std::format("{}){}{:<16}", slot, flag, item.get_display_name());
			}
		}
